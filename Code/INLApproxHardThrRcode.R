...
library(pracma)
INLApproxHardThrR<-function(X, M, Bi, S, EB, EBB, Beta, distance, gamma0s, eta0, Beta1, Sigma1, g1, e1, iter, maxit, thresh,
  ...
  ### the hardthresholding values are based on Alpha (beta_jk*sigma)
  for (nodej in 1:p){
  ...
  for (nodej in 1:p){
    ...
    hardthresh=HardThresholdApproxjC(inibetaj, inisigmaj, seqbetaj, betaK, X, M, EB, EBB,  
                                     indexjk, indexjkk, S, distance,  gamma1, eta0, lambda1, coeff1, cutnum, nodej-1)
    
    
    sigmajs=hardthresh$sigmajs
    
    Likelihoodi=hardthresh$Likelihoodi
    
    # llj=hardthresh$Likeli
    
    
    Nj=apply(Likelihoodi,2,function(x){sum(x>0)})
    llj=apply(Likelihoodi,2, function(x){sum(log(x[x>0]))})
    ...
    EBIC=-2*llj+E*log(Nj)+2*hyper*E*log(p-1)
