...
// [[Rcpp::export]]
Eigen::VectorXd linearApproxC(Eigen::MatrixXd di, Eigen::MatrixXd ci,  Eigen::VectorXd lambda,
  ...
  Eigen::MatrixXd noAddci=Eigen::MatrixXd::Ones(N,p);
  
  Eigen::MatrixXd noAddci2=Eigen::MatrixXd::Ones(N,p);
  
  
  
  for (i=0; i<N; ++i){
    for (j=0; j<p; ++j){
      //  if (std::abs(ci(i,j))>1.5) {ci(i,j)=100;}
      if (ci(i,j)<0){
        // indicatorc(i,j)=0;
        disignc(i,j)=-di(i,j);
        nci(i,j)=-ci(i,j);
      ...
      if (std::abs(ci(i,j))>1.4) {noAddci(i,j)=0;} 
      if (std::abs(ci(i,j))>10)  {noAddci2(i,j)=0;} 
    }
  }
  ...
  // for (i=0; i<N; ++i){
  //   for (j=0; j<p; ++j){
  //     if (std::abs(disignc(i,j)+2*a[i]*std::abs(ci(i,j)))>3.5) {noAddci(i,j)=0;}
  //   }
  // }
  
  // a|cj|
  Eigen::MatrixXd aC=Eigen::MatrixXd::Zero(N,p);
  
  for (i=0; i<N; ++i){
    aC.row(i)=a(i)*(ci.row(i)).cwiseAbs();
  }
  
  
  
  // sum|cj|
  Eigen::VectorXd sumC=Eigen::VectorXd::Zero(N);
  sumC=(ci.cwiseAbs()).rowwise().sum();
  
  //  pk*prod_ ...
  Eigen::MatrixXd pexp=Eigen::MatrixXd::Ones(N,m);
  
  Eigen::VectorXd constant=Eigen::VectorXd::Zero(N);
  
  Eigen::MatrixXd value;
  
  Eigen::VectorXd valuei;
  
  // k=0; 
  for (k=0; k<m; ++k){
    Eigen::MatrixXd prodj=Eigen::MatrixXd::Ones(N,p);
    Eigen::MatrixXd addExp=Eigen::MatrixXd::Zero(N,p);
    //  prodj=prodj.array()+(disignc.array()+2*a.array()*(ci.array().abs())-lambda(k)*(ci.array().abs())).exp();
    value=Eigen::MatrixXd::Zero(N,p);
    //  valuei=Eigen::VectorXd::Zero(N);
    
    value=disignc+2*aC-lambda(k)*(ci.cwiseAbs());
    
    // valuei=value.rowwise().maxCoeff();
    // 
    // for (i=0; i<N; ++i){
    //   for (j=0; j<p; ++j){
    //     
    //     if (value(i,j)==valuei[i]){
    //       value(i,j)=value(i,j)-Mbeta[i];
    //       prodj(i,j)=exp(-Mbeta[i]);
    //     }
    //     
    //   } // end of j
    // } // end of i
    ...
    addExp=value.array().exp();
    
    addExp=addExp.array()*(noAddci2.array());
    // 
    if (k<7){
      addExp=addExp.array()*(noAddci.array());
    }
    
    
    prodj=prodj+addExp;
    
    // prod_j=1^p (1+exp(dj*sign(cj)+2a|cj|-lambda_k|cj|))
    Eigen::VectorXd prod=prodj.rowwise().prod();
  ...
  for (i=0; i<N; ++i){
    // if (sumC(i)<2){
    //   sump(i)=coeff1.dot(pexp.row(i));
    // }
    // if (sumC(i)>2 & sumC(i)<3){
    //   sump(i)=coeff2.dot(pexp.row(i));
    // }
    // if (sumC(i)>3 & sumC(i)<4){
    //   sump(i)=coeff3.dot(pexp.row(i));
    // }
    // if (sumC(i)>4 & sumC(i)<5){
    //   sump(i)=coeff4.dot(pexp.row(i));
    // }
    sump(i)=coeff.dot(pexp.row(i));
    
  }
  
  
  Eigen::VectorXd front=(-a.array()*a.array()+(dci.array()).rowwise().sum()).exp();
  constant=front.array()*sump.array();
  
  return constant;
  
  
  // return(List::create(Named("front")=front, Named("sump")=sump,Named("a")=a,Named("sumC")=sumC,
...
/********* E-step, expectation ********/
// [[Rcpp::export]]
List Estep1jC(Eigen::MatrixXd Beta, Eigen::MatrixXd X, Eigen::MatrixXd M,
  ...
  return(List::create(Named("indexAct")=indexAct, Named("indexjk")=indexjk,
                      Named("XBM")=XBM,Named("UMi")=UMi,Named("UMMi")=UMMi,
...
List ExpectApproxjC(Eigen::MatrixXd Beta, Eigen::MatrixXd X, Eigen::MatrixXd M,
  ...
  for (k=0; k<p; ++k){
    if (k!=j){
      ...
      // Eigen::VectorXd Mbeta1=(UMi.col(jk).array())*(UMi.col(jk).array())/sigma(j)*0.5;
      // 
      // Eigen::VectorXd Mbeta2=(M.col(j).array())*(M.col(j).array())/sigma(j)*0.5;
      
      
      constantb1=linearApproxC(db1, cb,  lambda, coeff);
      
      constantb2=linearApproxC(db2, cb,  lambda, coeff);
      ...
      if (k<(p-1)){
        for (k2=k+1; k2<p; ++k2){
          if (k2!=j){
            ...
            // Eigen::VectorXd Mbetab1=(UMMi.col(jkk).array())*(UMMi.col(jkk).array())/sigma(j)*0.5;
            // 
            // Eigen::VectorXd Mbetab2=(UMi.col(jk).array())*(UMi.col(jk).array())/sigma(j)*0.5;
            // 
            // Eigen::VectorXd Mbetab3=(UMi.col(jk2).array())*(UMi.col(jk2).array())/sigma(j)*0.5;
            // 
            // Eigen::VectorXd Mbetab4=(M.col(j).array())*(M.col(j).array())/sigma(j)*0.5;
            
            constant1=linearApproxC(d1, c, lambda, coeff);
            
            constant2=linearApproxC(d2, c, lambda, coeff);
            
            constant3=linearApproxC(d3, c, lambda, coeff);
            
            constant4=linearApproxC(d4, c, lambda, coeff);
...
// [[Rcpp::export]]
List LikeliObsApproxjC(Eigen::MatrixXd Beta, Eigen::MatrixXd X, Eigen::MatrixXd M,
                       Eigen::MatrixXd indexAct, Eigen::MatrixXd indexjk, Eigen::MatrixXd indexjkk, 
                       Eigen::MatrixXd XBM, 
                       Eigen::VectorXd sigma, Eigen::MatrixXd S, Eigen::MatrixXd distance,  double gamma, double eta,  Eigen::VectorXd lambda,
                       Eigen::VectorXd coeff, int nodej){   
  ...
  constant=linearApproxC(db, cb, lambda, coeff);
...
List HardThresholdApproxjC(Eigen::VectorXd inibetaj, double inisigmaj, Eigen::VectorXd seqbetaj, Eigen::MatrixXd betaK, Eigen::MatrixXd X, Eigen::MatrixXd M, Eigen::MatrixXd EB, Eigen::MatrixXd EBB, 
                           Eigen::MatrixXd indexAct, Eigen::MatrixXd indexjk, Eigen::MatrixXd indexjkk, Eigen::MatrixXd S, Eigen::MatrixXd distance,  double gamma, double eta,  Eigen::VectorXd lambda,
                           Eigen::VectorXd coeff, int cutnum, int nodej){   
  ...
  for (rk=0; rk<cut; ++rk){
    ...
    Likelihoodi=(prodprobj.array())*exp(-(M.col(j).array())*(M.col(j).array())/sigmaj*0.5)/sqrt(2*M_PI*sigmaj);
